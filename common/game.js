...
module.exports = Class({
  ...
  advanceState: function() {
    ...
    if (this.tick % g.TICKS_PER_SECOND == 0) {
      // HACK: Assume that we don't need states older than 2s and clear
      // them.
      for (var tick in this.stateHistory) {
        if (tick + g.intdiv(2000,g.MS_PER_TICK) < this.tick) {
          delete this.stateHistory[tick];
        }
      }
    }

    if (this.tick % g.TICKS_PER_SECOND == g.intdiv(g.TICKS_PER_SECOND,2)) {
