...
var applyCommand = function(gameId, playerId, ms, commandlist) {
	...
	io.sockets.emit("serverinput",{ms:ms, commands:commandlist, playerId:playerId});
};
		...
		setTimeout(function() {
			...
			socket.set('playerId', playerId, function() {
				socket.set('gameId', data.gameid, function() {
					...
					socket.emit('serverinit', {startTime:game.startTime, tick:game.tick, stateHistory:clone(game.stateHistory), commandHistory:clone(game.commandHistory)});
